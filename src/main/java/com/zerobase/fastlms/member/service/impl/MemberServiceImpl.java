...
public class MemberServiceImpl implements MemberService {
        ...
        if (!CollectionUtils.isEmpty(list)) {
            ...
            for(MemberDto x : list) {
                ...
                List<LoginHistory> loginHistories =
                        loginHistoryRepository.findByLoginIdOrderByLoginDtDesc(x.getUserId());
                if (!CollectionUtils.isEmpty(loginHistories)) {
                    LoginHistory history = loginHistories.get(0);
                    x.setLoginDt(history.getLoginDt());
                }
    ...
    @Override
    public ServiceResult logLoginHistory(String userId, String userAgent, String clientIp) {
        if (userId == null || userId.length() == 0){
            return new ServiceResult(false, "유효한 로그인이 아닙니다.");
        }
    ...
    @Override
    public List<LoginHistoryDto> getMemberLoginHistory(String userId) {
        List<LoginHistory> loginHistories = loginHistoryRepository.findByLoginId(userId);
        List<LoginHistoryDto> list = new ArrayList<>();
        if (!CollectionUtils.isEmpty(loginHistories)) {
            list = LoginHistoryDto.of(loginHistories);
        }
